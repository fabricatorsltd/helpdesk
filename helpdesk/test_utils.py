...
# name: (level, response_time, resolution_time)
TEST_TICKET_PRIORITIES = {
    "Urgent": ("Urgent", 60 * 30, 60 * 60 * 2),
    "High": ("High", 60 * 60 * 1, 60 * 60 * 4),
    "Medium": ("Medium", 60 * 60 * 8, 60 * 60 * 24),
    "Low": ("Low", 60 * 60 * 24, 60 * 60 * 72),
}
...
def make_new_sla():
    ...
    sla_doc.priorities = []
    for priority, (_level, response, resolution) in TEST_TICKET_PRIORITIES.items():
        sla_doc.append(
            "priorities",
            {
...
def make_test_ticket_priorities():
    ...
    for priority, (level, _response, _resolution) in TEST_TICKET_PRIORITIES.items():
...
def add_test_priorities_to_sla(sla_name: str):
    ...
    for priority in missing:
        _level, response, resolution = TEST_TICKET_PRIORITIES[priority]
        sla_doc.append(
            "priorities",
            {
                "doctype": "HD Service Level Priority",
...
def _set_sla_priorities(sla_doc, priorities: list[str]):
    sla_doc.priorities = []
...
def make_sla(
    ...
    if frappe.db.exists("HD Service Level Agreement", sla_name):
        ...
        sla_doc.rank = rank
        if priorities:
            _set_sla_priorities(sla_doc, priorities)
